...
> 實際發生:使用者在一個空資料庫上開 `/login`,系統無聲轉址到 `/register`,
> 他照著填了一組帳密按下去,就成了管理員。`/login` 本身沒有問題(POST 只做密碼比對,
> 從不建立帳號),但「第一個註冊的人即管理員 + 系統開在內網上」等於
> **誰先打開那個網址誰就是管理員**。原本只有啟動視窗的文字提醒,
> 對「已經連得到網頁的人」沒有任何約束力。

- [ ] **建立第一個管理員必須輸入設定碼**,設定碼只出現在主機(啟動視窗與主機上的
      `setup_code.txt`),純從網頁端連進來的人拿不到。
  - 驗證:不帶 `setup_code` POST `/register` 回 200 且含「設定碼不正確」,使用者數維持 0;
    帶錯誤設定碼同樣被拒。
- [ ] **帶正確設定碼可建立管理員**,建立後設定碼檔案自動刪除。
  - 驗證:帶正確碼 POST 回 302、該帳號 `is_admin=1`、`setup_code.txt` 不再存在。
- [ ] **設定碼避開容易看錯的字元**(0/O/1/I),因為要用眼睛從視窗抄到瀏覽器。
  - 驗證:產生的設定碼不含 `0`、`O`、`1`、`I`。
- [ ] **命令列建立管理員(`--setup-admin`)同樣視為已完成設定**,會清掉設定碼。
  - 驗證:執行後 `setup_code.txt` 不存在。
- [ ] **註冊頁必須明白告知「這裡不是登入畫面」**,並說明第一個帳號就是管理員。
  - 驗證:空資料庫下 `curl -L /login` 的內容含「這裡不是登入畫面」。
- [ ] **註冊頁要顯示目前使用的資料庫路徑**,讓誤開空資料庫的人當場看得出來。
  - 驗證:該頁內容含目前 `INVENTORY_DB` 的絕對路徑,以及「你開到的是一個空的資料庫」。
- [ ] **已有帳號後仍然完全關閉自助註冊**(帶設定碼也不行)。
  - 驗證:有帳號後帶任何設定碼 POST `/register` 皆回「不開放自助註冊」,使用者數不變。

### 找得到自己的資料

> 同一件事的另一半:使用者把新版解壓縮到別的資料夾、在那裡啟動,看到的是全新的空系統,
> 會以為資料不見了。實際上舊資料好好地留在原本的資料夾。

- [ ] **`--where` 回報目前使用的資料庫路徑與內容筆數**(帳號/商品/異動),以及備份數量。
  - 驗證:對有資料的 DB 執行,輸出的商品筆數與資料庫實際筆數一致。
- [ ] **在錯的資料夾執行時要明講「這裡本來沒有資料庫」**,而不是只顯示 0 筆。
  - 驗證:指向不存在的路徑執行,輸出含「本來沒有資料庫」與「你的資料沒有不見」。
  - 註:`init_db()` 在模組載入時就會把檔案建出來,所以「本來有沒有」必須在它之前記錄。
- [ ] **提供不必打指令的雙擊版本**(`check_data.bat`)。
  - 驗證:檔案存在、為 CRLF、內容含 `--where`。

### 忘記密碼要有自救路徑

> 實際發生:使用者忘記管理員密碼。密碼以 `werkzeug` 單向雜湊儲存,原文從未保存,
> **任何人都無法查出舊密碼**。而當時 `--setup-admin` 只在「完全沒有帳號」時才動作,
> 等於關閉自助註冊之後,唯一的管理員一旦忘記密碼就永遠進不去。
> 這個風險在第四階段的規劃就被寫下來過,但只補了「管理員替別人重設」,
> 沒補「唯一的管理員自己忘記」。

- [ ] **`--list-users` 列出所有帳號與角色**,不顯示任何密碼相關內容。
  - 驗證:`python inventory_app.py --list-users` 列出帳號、角色、建立時間;
    輸出不含 `password` 或 `hash` 字樣。
- [ ] **`--reset-password <帳號> <新密碼>` 可重設密碼**,重設後可用新密碼登入。
  - 驗證:重設後 POST `/login` 帶新密碼回 302,帶舊密碼回 200 且含「帳號或密碼錯誤」。
- [ ] **重設不改變該帳號的權限**。
  - 驗證:重設管理員後,該帳號仍可開啟 `/audit`(回 200)。
- [ ] **帳號打錯時列出實際存在的帳號**(忘記密碼的人常常連帳號也忘了)。
  - 驗證:對不存在的帳號執行,輸出含現有帳號名稱且結束碼為 1。
- [ ] **新密碼仍受密碼長度規則約束**。
  - 驗證:以少於 8 碼的新密碼執行,輸出「密碼至少 8 碼」且結束碼為 1。
- [ ] **重設會寫入稽核紀錄**。
  - 驗證:`audit_log` 出現 action 為「重設密碼」的資料列。
- [ ] **提供不必打指令的雙擊版本**(`reset_password.bat`),會先列出帳號再詢問。
  - 驗證:檔案存在、為 CRLF、內容含 `--list-users` 與 `--reset-password`,
    且在沒有 `inventory.db` 時給出明確指引而非直接失敗。
- [ ] **指南要說明「舊密碼查不回來」與安全邊界**(只能在主機本機執行)。
  - 驗證:`公司內網架站指南.md` 含「忘記密碼」小節,並說明查不出舊密碼的原因。

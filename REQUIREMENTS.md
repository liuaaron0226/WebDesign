...
- [ ] **不得使用 emoji 當功能圖示**:圖示一律為內嵌線條 SVG,才能跟著配色走、在各系統長相一致。
  - 驗證:登入後首頁 `grep -c 'qa-icon"><svg\|qa-icon qa-[a-z]*"><svg'` ≥ 6;首頁快速動作區不含 emoji 字元。
- [ ] **按鈕有實體感與完整狀態**:主按鈕需有漸層/內光/陰影,並具備 hover、active(按下位移)、focus-visible(鍵盤可見焦點)三種狀態。
  - 驗證:`<style>` 含 `input[type=submit]:active` 的 `transform: translateY` 規則、`:focus-visible` 的 `outline` 規則、主按鈕的 `linear-gradient`。
- [ ] **層次以陰影色票統一管理**:陰影用藏青而非純黑,並定義為 `--sh-1/--sh-2` 等變數重複使用。
  - 驗證:`<style>` 含 `--sh-1`、`--sh-2`,且 `.quick-actions a` 使用 `var(--sh-`。
...
- [ ] **表單頁不得在寬容器中留下大片空白**:純表單頁的容器寬度需收窄。
  - 驗證:`<style>` 含 `.container:has(> form` 的 max-width 規則;截圖檢視。
...
- [ ] **首頁顯示採購狀態總覽**:一眼看到「已下訂 / 已出貨 / 已到貨待驗」各有幾張單、幾個品項。
  - 驗證:登入後首頁含「採購在途」區塊與三個狀態的數字;數字與資料庫一致。
- [ ] **商品列表新增「在途」欄**:顯示該料已下訂但尚未入庫的數量。
  - 驗證:首頁表頭含「在途」;某料有未收足的訂單時該欄顯示正確差額,無訂單時顯示 `—`。

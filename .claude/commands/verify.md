...
2. **啟動兩個 app**:確認依賴已安裝(必要時 `pip install --ignore-installed -r requirements.txt`),然後以背景執行啟動:
   ...
   - 庫存管理系統(每輪都要先清掉測試 DB 取得乾淨狀態):
     `rm -f /tmp/verify_inventory.db /tmp/inv_cookies.txt && INVENTORY_DB=/tmp/verify_inventory.db PORT=5002 python inventory_app.py`
...
5. **清理**:結束前務必關閉兩個背景 app process(`kill` 各 PID),並刪除測試過程產生的暫存檔(`/tmp/verify_inventory.db`、`/tmp/inv_cookies.txt`、測試用 CSV 與 input.txt 等)。
